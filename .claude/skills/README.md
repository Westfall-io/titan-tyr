...
| [`update-contract`](./update-contract/SKILL.md)            | Soft-metadata PUT on contracts (#52, #53, #54). Sets / clears the `project` tag and backfills `created_by_actor` (first-write-wins from X-Actor) without going through propose/accept. Body / version / subtype / endpoints all flow through their dedicated propose/accept skills. |
...
| [`update-skills`](./update-skills/SKILL.md)                       | **Downstream-consumer skill.** Pulls the titan-tyr skill catalog into a non-titan-tyr repo via `gh api`, namespacing each skill's helper scripts under `.claude/skills/<name>/scripts/`. Refuses to run inside titan-tyr itself. |
