...
description: Promote an open titan-tyr template proposal to the new active version. Templates span the build/runtime + contract group (software, container, image, pod, compose, interaction, binding, connection) and the K8s catalog group added in #91 (deployment, statefulset, service, ingress, secret, configmap, job). Use when the user wants to land a previously-proposed template change — e.g. "accept the template proposal", "promote 2.0.0-rc1 to active", "make this the new template". Lists open proposals, confirms which one to accept, and POSTs to /templates/{kind}/proposals/{version}/accept. Acceptance changes what every caller sees on `GET /templates/{kind}` — confirm before submitting.
---
