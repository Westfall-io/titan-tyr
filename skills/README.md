...
| [`update-contract`](./update-contract/SKILL.md)            | Soft-metadata PUT on contracts (#52, #53, #54). Sets / clears the `project` tag and backfills `owner_actor` (first-write-wins from X-Actor) without going through propose/accept. Body / version / subtype / endpoints all flow through their dedicated propose/accept skills. |
