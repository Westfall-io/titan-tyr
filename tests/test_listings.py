...
class TestSoftwareList:
    ...
    async def test_single_page(self, client):
        for name in ("svc-a", "svc-b", "svc-c"):
            await _register_software(client, name)
        r = await client.get("/software")
        assert r.status_code == 200
        body = r.json()
    ...
    async def test_paginates_with_cursor(self, client):
        ...
        page1 = (await client.get("/software", params={"limit": 2})).json()
        assert len(page1["results"]) == 2
        assert page1["next"] is not None

        page2 = (await client.get("/software", params={"limit": 2, "after": page1["next"]})).json()
        assert len(page2["results"]) == 2
        assert page2["next"] is not None

        page3 = (await client.get("/software", params={"limit": 2, "after": page2["next"]})).json()
    ...
    async def test_invalid_cursor_rejected(self, client):
        r = await client.get("/software", params={"after": "not-base64-json"})
        assert r.status_code == 422

    async def test_limit_out_of_range(self, client):
        r = await client.get("/software", params={"limit": 0})
        assert r.status_code == 422
        r = await client.get("/software", params={"limit": 101})
        assert r.status_code == 422
...
class TestContractListMode:
    ...
    async def test_single_page(self, client):
        ...
        await _register_software(client, "c")
    ...
    async def test_paginates(self, client):
        ...
        for n in names:
            await _register_software(client, n)
    ...
    async def test_search_mode_unchanged(self, client):
        ...
        await _register_software(client, "s1")
        await _register_software(client, "s2")
...
class TestSoftwareContractsListPagination:
    async def test_paginated(self, client):
        await _register_software(client, "hub")
        for spoke in ("sp1", "sp2", "sp3"):
            await _register_software(client, spoke)
            await _register_contract(client, "hub", spoke)
            await asyncio.sleep(0.01)
        page1 = (await client.get("/software/hub/contracts", params={"limit": 2})).json()
        assert page1["software"] == "hub"
        assert len(page1["results"]) == 2
        assert page1["next"] is not None
        page2 = (
            await client.get("/software/hub/contracts", params={"limit": 2, "after": page1["next"]})
        ).json()
